...
(function(jq){
	...
	posterTvGrid.prototype = {
		reset: function(options){
			...
			if(parseInt(this.body.outerWidth())>1255 || navigator.userAgent.indexOf('iPad') !== -1){
				this.options.width = 1350;	
			}else{
				this.options.width = 1050;	
			}
			...
			this.pageNowLeft = (this.options.width-676)/2
			this.drawContent();
		},
		drawContent: function(){
			this.parent.empty();
			this.parent.css({width:this.options.width+"px", 
				height:this.options.height+"px",
				 position: "relative"});
			this.content = document.createElement("DIV");
			this.content.className = this.options.className;
			this.content.cssText = "width:"+
		...
		initContent: function(){
			...
			this.parent.find(".contentHolderUnit:nth-child("+this.pageNow+")").css({width:'720px',height:'500px', opacity: 1, left: this.pageNowLeft+'px', zIndex: 3, marginTop: 0});
			...
			this.parent.find(".contentHolderUnit:nth-child("+pre+")").css({opacity: 1, left: this.preLeft+'px',height: '400px', width: '600px', zIndex: 0, marginTop: '53px'});
			...
			this.parent.find(".contentHolderUnit:nth-child("+next+")").css({opacity: 1, left: this.nextLeft+'px',height: '400px', width: '600px', zIndex: 0, marginTop: '53px'});
			...
			var run = function(page, dir, t){
				...
				if(pre != _this.pageNow && next != _this.pageNow){
					...
					_this.parent.find(".contentHolderUnit:nth-child("+nowpre+")").animate({
						...
						left:_this.options.width/2+'px', zIndex:0, 
						marginTop: '135px'}, t);
					...
					_this.parent.find(".contentHolderUnit:nth-child("+nownext+")").animate({
						width:'0px',height:'0px', opacity: 0, left:_this.options.width/2+'px',
						 zIndex:0, marginTop: '135px'}, t);
				}
				if(dir == 'left'){					
					...
					_this.parent.find(".contentHolderUnit:nth-child("+pre+")").animate({
						opacity: 1, left: _this.preLeft+'px', 
						height: '400px', width: '600px', zIndex: 2, 
						marginTop: '53px'}, t);
					...
					_this.parent.find(".contentHolderUnit:nth-child("+page+")").animate({
						opacity: 1, left: _this.pageNowLeft+'px',height: '500px', width: '720px',
						 zIndex: 3, marginTop: '0'}, t);
					...
					_this.parent.find(".contentHolderUnit:nth-child("+next+")").css({
						opacity: 0, left: _this.nextNLeft+'px', 
						height: '400px', width: '600px', zIndex: 2, 
						marginTop: '53px'});
					_this.parent.find(".contentHolderUnit:nth-child("+next+")").animate({
						opacity: 1, left: _this.nextLeft+'px', 
						height: '400px', width: '600px', zIndex: 2, 
						marginTop: '53px'}, t);
					...
					_this.parent.find(".contentHolderUnit:nth-child("+preP+")").animate({
						height: '400px', width: '600px', opacity: 0, left:_this.preNLeft+'px', 
						zIndex:0, marginTop: '53px'},t, "", 
						function(){_this.locked=false;});
					...
					_this.parent.find(".contentHolderUnit:nth-child("+next+")").animate({
						opacity: 1, left: _this.nextLeft+'px',height: '400px', width: '600px',
						 zIndex: 2, marginTop: '53px'}, t);
					_this.parent.find(".contentHolderUnit:nth-child("+next+") .elementOverlay").css({
						opacity:0.4});
					_this.parent.find(".contentHolderUnit:nth-child("+next+") .leftShadow").css({opacity:0});
					_this.parent.find(".contentHolderUnit:nth-child("+next+") .rightShadow").css({opacity:0});
					...
					_this.parent.find(".contentHolderUnit:nth-child("+page+")").animate({
						opacity: 1, left: _this.pageNowLeft+'px', height: '500px',
						 width: '720px', zIndex: 3, marginTop: '0px'}, t);
					...
					_this.parent.find(".contentHolderUnit:nth-child("+pre+")").css({
						opacity: 0, left: _this.preNLeft+'px',
						 height: '4000px',
						 width: '600px', zIndex: 2, marginTop: '53px'});
					_this.parent.find(".contentHolderUnit:nth-child("+pre+")").animate({
						opacity: 1, left: _this.preLeft+'px', height: '400px',

						 width: '600px', zIndex: 2, marginTop: '53px'}, t);
					_this.parent.find(".contentHolderUnit:nth-child("+pre+") .elementOverlay").css({opacity:0.4});
					...
					_this.parent.find(".contentHolderUnit:nth-child("+nextN+")").animate({
						width:'600px',height:'400x', opacity: 0,
						 left:_this.nextNLeft+'px', zIndex:0, marginTop: '53px'}, 
	...
	posterTvGrid.options = {
		...
		width : 1350,//最外层宽，需要使用的时候在传,默认由程序自动判断
		height : 540,//最外层高  
		delay : 7000,//滚动间隔（毫秒）
		speed : 800 //滚动速度毫秒
	};
